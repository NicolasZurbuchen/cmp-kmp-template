...
class PreviewTest {
    companion object {
        ...
        /**
         * Deliberately narrower than "Preview": `AppNamePreview` and `PreviewThemes` are the
    ...
    fun `Preview functions must render inside the preview harness`() {
        ...
        scope.files
            .withNameEndingWith(PREVIEW_SUFFIX)
            .assertTrue { file -> file.hasImport { it.name.endsWith(".AppNamePreview") } }
    }
     ...
     *
     * `AppNamePreview` is the opposite: it imports the theme and the palette, so it *is* the design
